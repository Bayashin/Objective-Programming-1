...
public class AichiToyotaAddress implements IKadai07_5{
    ...
    public Map<String, String> searchByCode(String postalCode) {
        Map<String, String> map = new HashMap<>(); //戻り値
        for (String key: POSTALCODE_MAP.keySet()) {  //マップが持つキーのセットを取得して回す
            if (key.startsWith(postalCode)){ //keyがpostalCodeで始まる場合
                String value = POSTALCODE_MAP.get(key); //valueに住所を取得
                map.put(key, value); //mapに追加
            }
        }
        return map;
    }
    ...
    public Map<String, String> searchByAddress(String addressText) {
        ...
        for (String key: POSTALCODE_MAP.keySet()){ //マップが持つキーのセットを取得して回す
            String value = POSTALCODE_MAP.get(key); //valueに住所を取得
            if (value.contains(addressText)){ //valueにaddressTextが含まれる場合
                map.put(key, value); //mapに追加
            }
        }
        return map;
...
//        Map<String, String> map = new HashMap<>();
        ...
        System.out.println("1:郵便番号検索　2:住所検索");
        int num = in_n.nextInt();
        switch (num) {
            case 1 -> {
                System.out.println("郵便番号");
                String str_c = in_s.nextLine();
                Map<String, String> map_c = app.searchByCode(str_c);
                for (String key : map_c.keySet()) {
                    String value = map_c.get(key);
                    System.out.println(key + ":" + value);
                }
            }
            case 2 -> {
                System.out.println("住所");
                String str_a = in_s.nextLine();
                Map<String, String> map_a = app.searchByAddress(str_a);
                for (String key : map_a.keySet()) {
                    String value = map_a.get(key);
                    System.out.println(key + ":" + value);
                }
            }
